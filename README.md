...
1. **Statistics**: Tabular & Graphical Form (`lessons/data-presentation.html`): syllabus 4.1 statistics terminology,
   ...
   then fill-in-the-blank frequency, cumulative frequency, boundary, midpoint and pie chart tables, plus endless
   random tables) → Mean, Mode, Median, Range, Variance & Standard Deviation (`statistics-notes.html`) → Statistics Lab (`lessons/statistics-lab.html`):
